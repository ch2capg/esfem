/*! \file iodof.h
    ...
    \brief Input and output for finite element function
     ...
     The class defined in this header gives an interface for input and/or output of 
     nodal values from a dune finite elemente function.
...
public:
  ...
  void operator()(const DiscreteFunction& fe);	
private:
  std::string filename;
  IO_direction io_dir;
  size_t f_no;
...
template<typename DiscreteFunction>
void IO_dune_fem::operator()(const DiscreteFunction& fe){
  ...
  case IO_direction::read:
    ...
    for(auto it = fe.dbegein(); it != fe.dend(); ++it)
      ifs >> *it;
    break;
  default:
