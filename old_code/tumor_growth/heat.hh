...
namespace Problem{
  ...
  struct MCF_Model{
    ...
    explicit MCF_Model(const Dune::Fem::TimeProviderBase& time_provider) :
      tp {time_provider} {}
  ...
  struct Reg_MCF_Model{
    ...
    explicit Reg_MCF_Model(const Dune::Fem::TimeProviderBase& time_provider,
			   const double regularization_parameter) :
      tp {time_provider}, reg_alpha {regularization_parameter} {}

    // Pseudocode:
    // (Mⁿ + (α+τ)Aⁿ) Xⁿ⁺¹ = (Mⁿ + αAⁿ) Xⁿ
    ...
    void ran_to_ran(const Entity& entity, const Point& xx, const RangeType& u_x, 
		    RangeType& u_x_chi) const;
    template <typename Entity, typename Point>
    void reg_jac_ran_to_jac_ran(const Entity& entity, const Point& xx, 
			    const JacobianRangeType& du_x,
			    JacobianRangeType& a_xi_du_x) const;
  private:
    const Dune::Fem::TimeProviderBase& tp;
  ...
  // Implementation details for 'MCF_Model'
  template <typename FunctionSpace> 
  template <typename Entity, typename Point>
  inline void MCF_Model<FunctionSpace>::
  jac_ran_to_jac_ran(const Entity& entity, const Point& xx, 
		     const JacobianRangeType& du_x,
		     JacobianRangeType& a_xi_du_x) const{
    // DomainType xGlobal = entity.geometry().global(Dune::coordinate(xx));
  ...
  // Implementation details for 'Reg_MCF_Model'
  template <typename FunctionSpace> 
  template <typename Entity, typename Point>
  inline void Reg_MCF_Model<FunctionSpace>::
  jac_ran_to_jac_ran(const Entity& entity, const Point& xx, 
		     const JacobianRangeType& du_x,
		     JacobianRangeType& a_xi_du_x) const{
    // DomainType xGlobal = entity.geometry().global(Dune::coordinate(xx));
  ...
  template <typename Entity, typename Point>
  inline void Reg_MCF_Model<FunctionSpace>::
  reg_jac_ran_to_jac_ran(const Entity& entity, const Point& xx, 
			      const JacobianRangeType& du_x,
			      JacobianRangeType& a_xi_du_x) const{
    // DomainType xGlobal = entity.geometry().global(Dune::coordinate(xx));
    // xGlobal.size() == dim_domain == 3;
