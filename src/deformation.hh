...
public:
	...
	void evaluate (const DomainVector& x, RangeVector& y ) const{
		...
		// 	  (*evoMapPt)[std::array<double, 3> { {x[0], x[1], x[2]} }];
...
private:
	BDF::EvoMapType* evoMapPt = 0;	// use this if your surface evolves via an ODE
	Dune::Fem::TimeProviderBase* p_tp = 0;
};
...
: public Dune::DiscreteCoordFunction<double, 3, 
									 ...
									 >{
	typedef Dune::DiscreteCoordFunction<double, 
										3, 
										DeformationDiscreteFunction<DiscreteFunctionType>
										> BaseType;
