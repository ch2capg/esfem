...
struct Esfem::SecOrd_op::Init_data::Data{  
  const Explicit_initial_data* eid_ptr {nullptr};
  const Random_initial_data* rid_ptr {nullptr};
...
Esfem::SecOrd_op::Init_data::Init_data(const Grid::Grid_and_time& gt) try{
    d_ptr = new Data {gt};
}
...
try{
  eid_ptr = new Explicit_initial_data {gt};
 }
 catch(const std::exception&){
...
Esfem::SecOrd_op::Init_data::Data::Data(const Io::Parameter& p, const Growth type)
  :   dof_io_filename {dof_filename(p, type)}
{
  try{
    rid_ptr = new Random_initial_data {p, type};
  }
  catch(const std::exception&){
