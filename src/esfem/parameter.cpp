...
Parameter::PDE_data::PDE_data(int argc, char** argv,
  ...
  d_ptr = new Data {
    ...
    Dune::Fem::Parameter::
    getValue<std::string> ("fem.io.outputName", "../output/ALE_LiteratureExample-"),
...
std::ofstream& Parameter::err_stream(const Parameter::PDE_data& d){
  static std::ofstream ofs {d.error_log(), std::ios_base::app};
  return ofs;
}
